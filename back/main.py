...
from crawler import converter, categories, subcategories  # Assuming these are defined in your utils module
...
from pydantic import BaseModel
from typing import List
from utils.crawler import converter, categories, subcategories
...
# Define your FastAPI endpoints
@app.post("/items/", response_model=List[ItemResponse])
def create_items(item: ItemCreate, item_num: int = Query(...), db: Session = Depends(get_db)):
    category_name = categories[item.category_choice][0]
    subcategory_name = subcategories[category_name][item.subcategory_choice][0]
    converted_data = converter(item.category_choice, item.subcategory_choice, item_num)
